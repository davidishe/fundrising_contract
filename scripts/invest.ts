...
export async function run(provider: NetworkProvider) {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--testnet' && i + 2 < args.length) {
            contractAddressStr = args[i + 1] || '';
            amount = args[i + 2] || '';
            break;
        }
    }
    ...
    if (!contractAddressStr) {
        console.log('💡 Usage: npx blueprint run invest --testnet [contractAddress] [amount]');
        console.log('   Example: npx blueprint run invest --testnet EQDk8e_FQLcW0vbwBY8-CUe43reR4xhTqgTPsycq9zMIt4cc 1.0');
        console.log('');
        console.log('📝 You will be prompted for:');
        console.log('   1️⃣ CrowdfundingMaster contract address');
        console.log('   2️⃣ Investment amount in TON');
        console.log('');
        contractAddressStr = await promptUser('📋 Enter CrowdfundingMaster contract address: ');
    }
    ...
    console.log('');
        ...
        const currentPrice = await safeGet(() => crowdfundingMaster.getGetCurrentPrice(), 'getCurrentPrice');

        console.log('🧮 Getting Master Jetton Wallet address...');
        const masterJettonWallet = await safeGet(() => crowdfundingMaster.getGetMasterJettonWallet(), 'getMasterJettonWallet');
